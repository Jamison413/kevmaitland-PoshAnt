...
$spoUsers = @()
$spCredentials  = New-Object Microsoft.SharePoint.Client.SharePointOnlineCredentials($o365admin , $o365password)
...
ForEach($spoUser in ($sharepointUsers | ? {$msolUsers.UserPrincipalName -contains $_.LoginName.Replace("i:0#.f|membership|","")})){
    $userprofile = $people.GetPropertiesFor($spoUser.LoginName)
...
$userHash = [ordered]@{}
$msolUsers | % {$userHash.Add($_.UserPrincipalName,@($_,$null,$null))}
$spoUsers | % {$userHash[$_.AccountName.Replace("i:0#.f|membership|","")][1] = $_}
$mailUsers | % {$userHash[$_.MicrosoftOnlineServicesID][2] = $_}
...
#Unfuckulate it so that it can be scripted.
...
foreach($ant in $userHash.Keys){# | ?{$_.Community -ne "Contractors"}){
    ...
    elseif($userHash[$ant][0].Title -eq "Generic Mailbox" -or $userHash[$ant][0].Department -eq "Unspecified"){
    ...
    $sheet.Cells.Item($currentRow,12) = get-formattedTimeZone (Get-MailboxRegionalConfiguration -Identity $userHash[$ant][0].UserPrincipalName).TimeZone
