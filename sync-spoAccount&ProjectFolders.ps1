...
if ([string]::IsNullOrEmpty($MyInvocation.ScriptName)){
    $fullLogPathAndName = $logFileLocation+"sync-spoClientsProjectsFolders_$accountType`_FullLog_$(Get-Date -Format "yyMMdd").log"
    $errorLogPathAndName = $logFileLocation+"sync-spoClientsProjectsFolders_$accountType`_ErrorLog_$(Get-Date -Format "yyMMdd").log"
    }
else{
    $fullLogPathAndName = "$($logFileLocation+$MyInvocation.MyCommand)_$accountType`_FullLog_$(Get-Date -Format "yyMMdd").log"
    $errorLogPathAndName = "$($logFileLocation+$MyInvocation.MyCommand)_$accountType`_ErrorLog_$(Get-Date -Format "yyMMdd").log"
    }
if($PSCommandPath){
    $transcriptLogName = "$($logFileLocation+$(split-path $PSCommandPath -Leaf))_$accountType`_Transcript_$(Get-Date -Format "yyMMdd").log"
...
catch{log-error -myError $_ -myFriendlyMessage "Could not retrieve [$spoListName] to check whether it needs recaching" -fullLogFile $fullLogPathAndName -errorLogFile -doNotLogToEmail $true}
...
#This is the standard behaviour for Projects
        elseif(!$dirtyProject.PreviousName -and (!$dirtyProject.PreviousKimbleClientId -or $dirtyProject.PreviousKimbleClientId -eq $dirtyProject.KimbleClientId)){ 
            ...
            try{
                ...
                $duration = Measure-Command {$updatedProjectFolder = update-projectFolder -spoKimbleProjectList $pnpProjectList -spoKimbleProjectListItem $dirtyProject -clientCacheHashTable $kimbleAccountHashTable -arrayOfProjectSubfolders $listOfLeadProjSubFolders -recreateSubFolderOverride $recreateAllFolders -adminCreds $adminCreds -fullLogPathAndName $fullLogPathAndName}
