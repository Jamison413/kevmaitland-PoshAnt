...
#region  overwrite old PNP cmdlets due to versioning issues
...
#team sites
$requests = @()
Connect-PnPOnline -Url "https://anthesisllc.sharepoint.com/sites/TeamHub" -UseWebLogin
...
ForEach($item in $selectedRequests){
    $managedBy = Read-host "Declare master membership list (365 or AAD), default hit enter for 365"
    If(!$managedBy){
    $managedBy = "365"
    }
    Add-Member -InputObject $item -MemberType NoteProperty "siteClassification" -Value "Internal" -Force
}
...
Switch($currentRequest.siteClassification){
    "Internal"{
        ...
        #Site Name
        if($fullRequest.FieldValues.Site_x0020_Type -eq "Functional"){$displayName = $($fullRequest.FieldValues.Title)}
        else{$displayName = "$($fullRequest.FieldValues.Title) $($fullRequest.FieldValues.Site_x0020_Type)"}

        # Site Template Location
        $siteTemplate = "Candiate-Template-for-Team-Site-Landing-Page.aspx"
        $hubAssociation = "Team"
        ...
        #region Site Managers & Members
            ...
            $members = convertTo-arrayOfEmailAddresses ($managers + "," + $fullRequest.FieldValues.Members.Email) | sort | select -Unique
    ...
    "External"{
        ...
        #Site Name
        $displayName = $("External - $($fullRequest.FieldValues.Title)".Trim(" "))
        
        # Site Template Location
        $siteTemplate = "External-Site-Template-Candidate.aspx"
        $hubAssociation = "External"
        ...
        #region Site Managers & Members
                ...
                <#
                    #migration companies bodge
                ForEach($manager in $managers){
        
                    If($manager -notcontains "@anthesisgroup.com"){
                        $Email = get-graphUsers -tokenResponse $tokenResponse -filterCustomEq @{"mail" = $manager}
                        If(($thisEmail | measure-object).count -eq 1){
                            $managers += $Email.userPrincipalName
                            $managers = $managers.where{$_ -ne $manager}
                    }
                    Else{
                            write-host "I've found too many users" -ForegroundColor red
                    }
                    }
                }
                ForEach($member in $members){
        
                    If($member -notcontains "@anthesisgroup.com"){
                        $Email = get-graphUsers -tokenResponse $tokenResponse -filterCustomEq @{"mail" = $member}
                        If(($Email | measure-object).count -eq 1){
                            $members += $Email.userPrincipalName
                            $members = $members.where{$_ -ne $member}
                    }
                    Else{
                            write-host "I've found too many users" -ForegroundColor red
                    }
...
#region configure SPO
        ...
        copy-spoPage -sourceUrl "https://anthesisllc.sharepoint.com/sites/Resources-IT/SitePages/$($siteTemplate)" -destinationSite $newPnpTeam.SiteUrl -pnpCreds $365creds -overwriteDestinationFile $true -renameFileAs "LandingPage.aspx" | Out-Null
        ...
        Add-PnPHubSiteAssociation -Site $newPnpTeam.SiteUrl -HubSite "https://anthesisllc.sharepoint.com/sites/$($hubAssociation)Hub" | Out-Null
        Write-Verbose "Opening in browser"
        If($hubAssociation -eq "External"){
