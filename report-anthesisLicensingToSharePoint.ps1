...
function get-folderInLibrary($sitePath, $libraryName, $folderPath, $folderName){
    #Sanitise parameters
    $sitePath = format-path (sanitise-forSharePointUrl $sitePath)
    $sanitisedLibraryName = format-path (sanitise-forSharePointUrl (sanitise-forSharePointListName $libraryName))
...
function get-library($sitePath, $libraryName){
    #Sanitise parameters
    $sitePath = format-path (sanitise-forSharePointUrl $sitePath)
    $sanitisedLibraryName = format-path (sanitise-forSharePointUrl (sanitise-forSharePointListName $libraryName)) #The LibraryName cannot contain specific characters
    #Build and execute REST statement
...
function new-itemInList($sitePath,$listName,$predeterminedItemType,$hashTableOfItemData){
    ...
    #$metadata = "{ '__metadata': { 'type': '$predeterminedItemType' }, 'Title'='Test'}"
    try{
...
$logToScreen = $true
$logToFile = $true
$logfile = "C:\Scripts\Logs\record-anthesisLicensingToSharePoint_$(Get-Date -Format "yyMMdd").log"
...
$prettyLicenseNames = @{"AnthesisLLC:ENTERPRISEPACK" = "E3";"AnthesisLLC:EXCHANGEDESKLESS"="Kiosk";"AnthesisLLC:PROJECTPROFESSIONAL"="Project";"AnthesisLLC:STANDARDPACK"="E1";"AnthesisLLC:VISIOCLIENT"="Visio";"AnthesisLLC:WACONEDRIVESTANDARD"="OneDrive"}
...
$msolUsers = Get-MsolUser | ?{$_.Licenses.Count -gt 0}
...
$targetList = get-list -sitePath $sitePath -listName "SPOLicensing"
$timeStamp = Get-Date
...
foreach($upn in $userHash.Keys){
    foreach($license in $userHash[$upn][0].Licenses){
        ...
        $itemToAdd = @{Title=$userHash[$upn][0].DisplayName;UserPrincipalName=$upn;LicenseName=$licenseName;BusinessEntity=$businessEntity;TimeStamp=$timeStamp;Community=$userHash[$upn][0].Department;Country=$userHash[$upn][0].Country}
        new-itemInList -sitePath $sitePath -listName $targetList.Title -predeterminedItemType $targetList.ListItemEntityTypeFullName -hashTableOfItemData $itemToAdd
        }
    }
