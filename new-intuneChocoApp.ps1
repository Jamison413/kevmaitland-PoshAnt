...
$appFriendlyName = ""              #e.g. "Adobe Reader (managed)"
$appChocoName = ""                 #e.g. "AdobeReader"
$installedAppFileForDetection = "`${env:ProgramFiles}\Foxit Software\Foxit Reader" #e.g. "`${env:ProgramFiles(x86)}\Adobe\Acrobat Reader DC" #This needs to have the ` (backtick) before any $env: variables so that passes through the generic "$env:LOCALAPPDATA" rather than the specifc "C:\Users\KevMaitland\AppData\Local" (which won't work for most users)
