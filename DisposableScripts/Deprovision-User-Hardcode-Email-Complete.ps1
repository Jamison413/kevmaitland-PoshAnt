...
$userAdminPass = ConvertTo-SecureString (Get-Content C:\Users\antmin\Downloads\GroupBot.txt) 
...
sleep -Seconds 60
Write-Host "check"
$check = Get-MsolUser -UserPrincipalName $user
if(($check.DisplayName).ToCharArray()[0] -eq "Ω"){
$outcome = "We have deprovisioned account"
}
Else{
$outcome = "We had an issue deprovisioning account"
$errorMessage = $error[0].Message
}
#Send email
$body = "<HTML><BODY><p>Hi IT Team ,</p>
                <p>$($outcome) $($upnsString):</p>
                <p><b>Username :</b> $($userExoObject.UserPrincipalName)<br>
                <p>($errorMessage)<br>
                <p>Kind regards,<br>
                The IT Team</p>
                "
send-graphMailMessage -tokenResponse $tokenResponseSmtp -fromUpn "Shared_Mailbox_-_IT_Team_GBR@anthesisgroup.com" -toAddresses "Shared_Mailbox_-_IT_Team_GBR@anthesisgroup.com" -subject "Auto Deprovision" -bodyHtml $body 
