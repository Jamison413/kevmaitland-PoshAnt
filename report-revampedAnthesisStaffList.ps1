...
$sharePointAdminPass = ConvertTo-SecureString (Get-Content $env:USERPROFILE\Downloads\GroupBot.txt) 
...
foreach($ant in $userHash.Keys){# | ?{$_.Community -ne "Contractors"}){
    if($userHash[$ant][0].Title -eq "" -or $userHash[$ant][0].Title -eq $null -or $userHash[$ant][0].DisplayName -eq "Ω_Diana Correal"){
...
#Set the run dates
$sheet = $workbook.Sheets.Item('Anthesians')
$sheet.Range("A7") = $(Get-Date -Format u).Replace("Z","")+" (UTC)"
$sheet = $workbook.Sheets.Item('Contractors')
$sheet.Range("A7") = $(Get-Date -Format u).Replace("Z","")+" (UTC)"

#Delete the irrelevent worksheets
$WorkBook.worksheets.Item("Service Accounts").Delete()
