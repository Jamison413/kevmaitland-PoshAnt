...
function new-folderInLibrary($sitePath, $libraryName, $folderPath, $folderName){
    #$libraryName = $kimbleSupplierHashTable[$dirtyProject.KimbleSupplierId]
...
function new-itemInList($sitePath,$listName,$predeterminedItemType,$hashTableOfItemData){
    ...
    foreach($key in $hashTableOfItemData.Keys){
        if($hashTableOfItemData[$key].Length -gt 254){$formattedItemData += "`'$key`':`'$($hashTableOfItemData[$key].ToString().SubString(0,254).Replace("'","&#39"))', "}
        else{$formattedItemData += "`'$key`':`'$($hashTableOfItemData[$key].ToString().Replace("'","&#39"))', "}
        }
...
function log-error($myError, $myFriendlyMessage, $doNotLogToFile, $doNotLogToScreen, $doNotLogToEmail, $overrideErrorLogFile){
    if($logErrors){
        ...
        if(!$doNotLogToEmail -or $logErrorsToEmail){Send-MailMessage -To $mailTo -From $mailFrom -Subject "Error in update-SpoSuppliersFolders - $myFriendlyMessage" -Body ("$myError.") -SmtpServer $smtpServer}
        }
    }
